$.Cupid = function(x, y, w, h) {
	...
	this.CupidLoveTick = 6.9;
	this.CupidLoveElapsed = 0;

	this.Arrow = { 
		bounds: new $.Rectangle(0, 0, 140, 35),
		speed: 15
	};
...
$.Cupid.prototype.SetupParticles = function() {
	...
	this.ArrowParticleEngine.Spin = 3;
	...
	this.ArrowParticleEngine.Size = 5;
	this.ArrowParticleEngine.MaxSize = 8;
	...
	this.ArrowParticleEngine.Gravity = new $.Point(0, 10);
	...
	this.CupidParticleEngine.MinSpeed = 8;
	this.CupidParticleEngine.MaxSpeed = 10;
...
$.Cupid.prototype.Update = function() {
	...
	if (this.Stages.one === 1) {		
		...
		if (this.Arrow.bounds.Right + 100 < $.CanvasBounds.Left) {
		...
		this.Arrow.bounds.X -= this.Arrow.speed;
		var distance = this.Arrow.bounds.Centre.DistanceBetween($.CanvasBounds.Centre);
		if (distance <= 55)
		{
			this.ToggleStage(1);
			this.ArrowParticleEngine.Emit = false;
